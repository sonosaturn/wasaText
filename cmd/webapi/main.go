...
import (
	...
	"github.com/sonosaturn/wasatext/service/api"
...
// * creates and configure the loggithub.com/sonosaturn/wasatextger
...
func run() error {
	...
	// Normalize DB path to avoid confusion with relative paths.
	dbPath := cfg.DB.Filename
	if abs, aerr := filepath.Abs(dbPath); aerr == nil {
		dbPath = abs
	}
	logger.Infof("using SQLite DB file: %s", dbPath)
